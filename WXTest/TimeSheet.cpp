                    ...
                    if (ch == 27) {  // ESC key
                        ...
                        if (isValidEmployeeName(newName)) {
                            ...
                            if (rename(oldFileName.c_str(), newFileName.c_str()) != 0) {
                                cout << "Failed to rename timesheet file." << endl;
                            }
...
double computeHours(const std::string& timeIn, const std::string& timeOut) {
    ...
    if (sscanf_s(timeIn.c_str(), "%d:%d %2s",
        &h1, &m1, ampm1, (unsigned)_countof(ampm1)) != 3 ||
        sscanf_s(timeOut.c_str(), "%d:%d %2s",
            &h2, &m2, ampm2, (unsigned)_countof(ampm2)) != 3) {
